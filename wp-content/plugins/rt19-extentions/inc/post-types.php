...
if ( ! class_exists( 'RT_Custom_Posts' ) ) {
	...
	class RT_Custom_Posts{
		...
		public function __construct() {
			...
			add_action( 'init', array(&$this,'team'), 0);
			add_action( 'init', array(&$this,'testimonials'), 0);


			add_action( 'init', array(&$this,'is_product_showcase_active'), 0);
			add_action( 'init', array(&$this,'is_portfolio_active'), 0);
			add_action( 'init', array(&$this,'is_team_active'), 0);
			add_action( 'init', array(&$this,'is_testimonials_active'), 0);
			...
			if( is_admin()){
				...
				add_action('switch_theme', 'flush_rewrite_rules'); //flush rewrite rules when the theme switched
		...
		function portfolio(){
			...
			// Slug Names			
			$portfolio_slug              = get_option(RT_EXTENTIONS_SLUG."_portfolio_single_slug"); 		// singular portfolio item
			$portfolio_categories_slug   = get_option(RT_EXTENTIONS_SLUG."_portfolio_category_slug");		// portfolio categories 
		...
		function products(){
			...
			// Slug Names			
			$product_slug              = get_option(RT_EXTENTIONS_SLUG."_product_single_slug"); 		// singular product item
			$product_categories_slug   = get_option(RT_EXTENTIONS_SLUG."_product_category_slug");		// product categories 
		...
		function team(){
			...
			// Slug Names			
			$team_slug              = get_option(RT_EXTENTIONS_SLUG."_team_single_slug"); 
		...
		function testimonials(){
			...
			// Slug Names			
			$testimonial_categories_slug   = get_option(RT_EXTENTIONS_SLUG."_testimonial_category_slug");		// testimonial categories 
		...
		public function permalink_settings() {
			...
			if( ! $this->is_product_showcase_active() && ! $this->is_product_showcase_active() && ! $this->is_team_active() ){
				return ;
			}
		...
		public function rttheme_products_category_slug_input() {
			$base = esc_attr(get_option( RT_EXTENTIONS_SLUG."_product_category_slug" ));
			?>
		...
		public function rttheme_products_single_page_slug_input() {
			$base = esc_attr(get_option( RT_EXTENTIONS_SLUG."_product_single_slug" ));
			?>
		...
		public function rttheme_portfolio_category_slug_input() {
			$base = esc_attr(get_option( RT_EXTENTIONS_SLUG."_portfolio_category_slug" ));
			?>
		...
		public function rttheme_portfolio_single_page_slug_input() {
			$base = esc_attr(get_option( RT_EXTENTIONS_SLUG."_portfolio_single_slug" ));
			?>
		...
		public function rttheme_team_single_page_slug_input() {
			$base = esc_attr(get_option( RT_EXTENTIONS_SLUG."_team_single_slug" ));
			?>
		...
		public function rttheme_testimonial_category_slug_input() {
			$base = esc_attr(get_option( RT_EXTENTIONS_SLUG."_testimonial_category_slug" ));
			?>
		...
		public function save_permalink_settings() {
			...
			if ( isset( $_POST['rttheme_products_category_slug'] ) ) {
				update_option( RT_EXTENTIONS_SLUG."_product_category_slug", esc_attr(sanitize_html_class($_POST['rttheme_products_category_slug'])) );
			}

			if ( isset( $_POST['rttheme_products_single_page_slug'] ) ) {
				update_option( RT_EXTENTIONS_SLUG."_product_single_slug", esc_attr(sanitize_html_class($_POST['rttheme_products_single_page_slug'])) );
			}

			if ( isset( $_POST['rttheme_portfolio_category_slug'] ) ) {
				update_option( RT_EXTENTIONS_SLUG."_portfolio_category_slug", esc_attr(sanitize_html_class($_POST['rttheme_portfolio_category_slug'])) );
			}

			if ( isset( $_POST['rttheme_portfolio_single_page_slug'] ) ) {
				update_option( RT_EXTENTIONS_SLUG."_portfolio_single_slug", esc_attr(sanitize_html_class($_POST['rttheme_portfolio_single_page_slug'])) );
			}

			if ( isset( $_POST['rttheme_team_single_page_slug'] ) ) {
				update_option( RT_EXTENTIONS_SLUG."_team_single_slug", esc_attr(sanitize_html_class($_POST['rttheme_team_single_page_slug'])) );
			}

			if ( isset( $_POST['rttheme_testimonial_category_slug'] ) ) {
				update_option( RT_EXTENTIONS_SLUG."_testimonial_category_slug", esc_attr(sanitize_html_class($_POST['rttheme_testimonial_category_slug'])) );
			}			
		}
		...
		function check_woo_permalink_conflicts(){
			...
			//Slugnames
			$rt_theme_product_slug =  esc_attr(get_option( RT_EXTENTIONS_SLUG."_product_single_slug" ));
			$rt_theme_product_category_slug =  esc_attr(get_option( RT_EXTENTIONS_SLUG."_product_category_slug" ));
