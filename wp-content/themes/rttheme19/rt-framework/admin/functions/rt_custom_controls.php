 ...
 * @return $type
 */
function rt_custom_controls( $wp_customize ){
		...
		class RT_Customize_Media_Control extends WP_Customize_Control {
			...
			public function __construct( $manager, $id, $args = array() ) {
				...
				$this->statuses = array( '' => __( 'Default', 'rt_theme_admin' ) );
			...
			public function render_content() {
				...
				<label>
					...
					<div class="form_element upload">
						<input id="<?php echo $id; ?>" autocomplete="off" type="text" <?php $this->link(); ?> class="upload_field">  
						<button data-inputid="<?php echo $id ?>" class="button icon-upload rttheme_upload_button" type="button"><?php echo __('Upload','rt_theme_admin'); ?></button>
					</div>
				...
				<label>
					...
					<div class="customize-control-content">
						<div class="wp-picker-container">
							<a class="rt-color-control-result wp-color-result" tabindex="0" style="background-color: <?php echo $this->value(); ?>" title="<?php echo __("Select Color","rt_theme_admin"); ?>" data-current="<?php echo __("Current Color","rt_theme_admin"); ?>"></a>
							
							<span class="wp-picker-input-wrap">
								<input type="text" value="<?php echo $this->value(); ?>" <?php $this->link(); ?> class="wp-color-picker" data-default-color="<?php echo $this->default; ?>" style="display:none;">
								<input class="button button-small wp-picker-default rt-color-control-default" type="button" value="Default" style="display:none;">
							</span>

							<div class="rt-color-control" data-elemenet-id="<?php echo $this->id; ?>"></div>
						</div>
					</div>
						...
						<select <?php $this->link(); ?> <?php $this->input_attrs(); ?>>
							...
							foreach ( $this->choices as $value => $label )
								...
								switch ( $value ) {
									...
									default:
										echo '<option value="' . esc_attr( $this->esc_son($value) ) . '"' . selected( $this->value(), $value, false ) . ' ' . create_option_atts( $this->value() ) . '>' . $label . '</option>';	
										break;
								}
...
add_action( 'customize_register', 'rt_custom_controls' );
